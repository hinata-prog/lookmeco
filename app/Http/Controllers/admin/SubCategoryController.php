...
class SubCategoryController extends Controller
{
    ...
    public function store(Request $request){
        $validator = Validator::make($request->all(), [
            'name'=> 'required',
            'slug' => 'required|unique:sub_categories,slug,NULL,id,category_id,' . $request->input('category_id'),
        ...
        if ($validator->passes()){
            $subCategory = SubCategory::create(
                $request->only('name', 'slug', 'category_id', 'status', 'showHome'
            ));
    ...
    public function update($subCategoryId, Request $request){
        ...
        $validator = Validator::make($request->all(), [
            "name"=> "required",
            "slug" => "required|unique:sub_categories,slug," . $subCategory->id . ",id,category_id," . $subCategory->category_id,
            "status"=>"required",
            'showHome' => 'required',
            'category_id' => 'required|exists:categories,id'
        ]);
